...
### Step 1: Create own CA (Certificate Authority) and self-signed certificate

We need to follow these steps to generate self-signed certifcate for the secure gRPC connection.
...
I have also created a bash script `ssl_certs.sh` under the `certs` directory, that can run all these above commands and generate the required keys and certs. I am going to run this script where my TIG stack running with docker compose. 


### Step 2: Edit telegraf.conf to enable TLS
...
Copy the certs to the telegraf container 
